...
public class TypeSpecCustomizationTemplate : BasePromptTemplate
{
    ...
    {
        var referenceDocContent = File.ReadAllText(_referenceDocPath);
    ...
    private string BuildTaskInstructions(string referenceDocContent)
    {
        ...
        **TypeSpec Project Path:** {_typespecProjectPath}

        **Customization Request:**
        {_customizationRequest}
    ...
    private static string BuildTaskConstraints()
    {
        ...
        **TypeSpec Best Practices:**
        - Use augmentation decorators (@@) when modifying existing types
        - Add a file-level namespace (e.g., `namespace ClientCustomizations;`) if defining new types
        - Use scope parameter for language-specific customizations
        - Prefer scope negation ("!csharp") when excluding a single language
    ...
    private static string BuildOutputRequirements()
    {
        ...
        **Workflow:**
        ...
        4. Continue until all changes are successfully applied
